...
$( echo $flags | xargs -s 78 | sed 's/^/  /g' )
  ...
  hardened ssp -bindist -cdinstall -oci8 -pax_kernel
"
...
#
profile=$(eselect profile list | awk ' { print $2 } ' | grep -e "^default" -e "^hardened" | grep -v -e '/x32' -e "/developer" -e '/musl' -e "/uclibc" -e "/selinux" | sort --random-sort | head -n1)
