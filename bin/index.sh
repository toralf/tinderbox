...
function listStat()  {
  ...
  $(dirname $0)/whatsup.sh -d | recode ascii..html >> $tmpfile
  echo "<h3>packages per hour</h3>" >> $tmpfile
  $(dirname $0)/whatsup.sh -e | recode ascii..html >> $tmpfile
  echo "<h3>current task per image</h3>" >> $tmpfile
  $(dirname $0)/whatsup.sh -t | recode ascii..html >> $tmpfile
  echo "<h3>current emerge per image</h3>" >> $tmpfile
  $(dirname $0)/whatsup.sh -l | recode ascii..html >> $tmpfile
  echo -e "\n</pre>\n" >> $tmpfile
}
...
function listImages()  {
  ...
  ls ~tinderbox/img/ | recode ascii..html | xargs --no-run-if-empty -I{} echo '<a href="./{}">{}</a>' >> $tmpfile
  echo -e "</pre>\n" >> $tmpfile
}
