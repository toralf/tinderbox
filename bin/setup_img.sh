...
function ThrowUseFlags()  {
  ...
  cut -f2 -d ':' |\
  cut -f1 -d ' ' |\
  egrep -v -e '32|64|^armv|bindist|build|cdinstall|debug|gcj|hostname|kill|linguas|make-symlinks|minimal|monolithic|multilib|musl|nvidia|oci8|pax|prefix|qt4|tools|selinux|static|symlink|systemd|test|uclibc|vim-syntax' |\
  sort -u --random-sort |\
  head -n $(($RANDOM % $n)) |\
  sort |\
  while read flag
