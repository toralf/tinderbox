...
function ThrowUseFlags()  {
  ...
  cut -f2 -d ':' |\
  cut -f1 -d ' ' |\
  egrep -v -e '32|64|^armv|bindist|build|cdinstall|debug|gallium|gcj|ghcbootstrap|hostname|kill|libav|linguas|make-symlinks|minimal|monolithic|multilib|musl|nvidia|oci8|opencl|pam|pax|prefix|qt4|tools|selinux|ssl|static|symlink|systemd|test|uclibc|vaapi|vdpau|vim-syntax|vulkan|X' |\
  sort -u --random-sort |\
  head -n $(($RANDOM % $n)) |\
  sort |\
  while read flag
