...
function DropUseFlags()  {
  egrep -v -e '32|64|^armv|bindist|build|cdinstall|compile-locales|debug|forced-sandbox|gallium|gcj|ghcbootstrap|hardened|hostname|ithreads|kill|libav|libreoffice|libressl|libunwind|linguas|make-symlinks|malloc|minimal|monolithic|multilib|musl|nvidia|oci8|opencl|openssl|pax|perftools|prefix|tools|selinux|split-usr|static|symlink|systemd|test|uclibc|vaapi|vdpau|vim-syntax|vulkan'
}
...
OMP_NESTED=FALSE
OMP_NUM_THREADS=1
# RUSTFLAGS is set in make.conf (contains more than just -j1)
RUST_TEST_THREADS=1
RUST_TEST_TASKS=1
