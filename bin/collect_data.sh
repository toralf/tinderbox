...
{
  if [[ ${1-} == "reset" ]]; then
    find ~tinderbox/img/*/var/db/pkg/ -mindepth 3 -maxdepth 3 -ignore_readdir_race -path '*-MERGING-*' -prune -o -name "NEEDED.ELF.2"
  else
...
{
  if [[ ${1-} == "reset" ]]; then
    find ~tinderbox/img/*/var/db/pkg/ -mindepth 3 -maxdepth 3 -ignore_readdir_race -path '*-MERGING-*' -prune -o -name "NEEDED" -print0 |
      xargs -0 -r grep -H . |
      sed -e 's,^/home/tinderbox/.*/.*/var/db/pkg/,,' -e 's,/NEEDED:, ,'
  else
    cat ~tinderbox/img/needed.txt
    find ~tinderbox/run/*/var/db/pkg/ -mindepth 3 -maxdepth 3 -ignore_readdir_race -path '*-MERGING-*' -prune -o -name "NEEDED" -cmin -65 -print0 |
      xargs -0 -r grep -H . |
      sed -e 's,^/home/tinderbox/.*/.*/var/db/pkg/,,' -e 's,/NEEDED:, ,'
  fi
} 2>/dev/null |
